...
func capture() -> void:
	...
	root.get_texture().get_image().save_png("res://docs/driving.png")
	...
	await RenderingServer.frame_post_draw
	root.get_texture().get_image().save_png("res://docs/landmark.png")
	game.car.recover(District.DESTINATION)
	await create_timer(1).timeout
	await RenderingServer.frame_post_draw
	root.get_texture().get_image().save_png("res://docs/discovery.png")
	game.close_card()
	...
	await RenderingServer.frame_post_draw
	root.get_texture().get_image().save_png("res://docs/places.png")
	game.queue_free()
	await process_frame
